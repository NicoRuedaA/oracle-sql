...
select titol,
       CONCAT(CONCAT(titol, ' - '), exemplars) as titol_llibre
  from LLIBRE;


select cognoms as cognom1
from autor;

SELECT SUBSTR(cognoms, INSTR(cognoms, ' ')) FROM autor;
SELECT SUBSTR(cognoms, 1, INSTR(cognoms, ' ')) FROM autor;

select SUBSTR(cognoms, 1, INSTR(cognoms, ' ' )-1) as cognom1,
      SUBSTR(cognoms, INSTR(cognoms, ' ')+1) as cognom2
  from autor;

SELECT UPPER(SUBSTR(REPLACE(cognoms, ' ', ''), 1, 10)) as no_espai_maj from autor;

SELECT UPPER(nom) as noms from autor  WHERE LENGTH(nom) >= 6 ;
